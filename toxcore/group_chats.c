 ...
 * the sanctions list version, the topic version, and the number of group TCP nodes
 */
#define GC_PING_PACKET_DATA_SIZE (sizeof(uint32_t) * 5)
...
static int validate_gc_peer_role(const GC_Chat *chat, uint32_t peernumber)
{
    ...
    switch (chat->group[peernumber].role) {
        ...
        case GR_MODERATOR: {
            if (mod_list_index_of_sig_pk(chat, SIG_PK(chat->gcc[peernumber].addr.public_key)) == -1)
                return -1;

            break;
        }
        case GR_USER: {
            if (sanctions_list_is_observer(chat, chat->gcc[peernumber].addr.public_key))
                return -1;
        ...
        case GR_OBSERVER: {
            ...
            if (!sanctions_list_is_observer(chat, chat->gcc[peernumber].addr.public_key) && peernumber != 0)
                return -1;
...
static void self_gc_connected(GC_Chat *chat)
{
    ...
    chat->last_tcp_nodes_check = unix_time();
}
...
static unsigned int new_gc_tcp_nodes(const Messenger *m, GC_Chat *chat);
 ...
 */
static int group_announce_request(GC_Session *c, GC_Chat *chat)
{
...
uint16_t gc_copy_peer_addrs(const GC_Chat *chat, GC_PeerAddress *addrs, size_t max_addrs)
{
    ...
    for (i = 1; i < chat->numpeers && num < max_addrs; ++i) {
...
uint16_t gc_copy_group_tcp_nodes(const GC_Chat *chat, Node_format *nodes, size_t max_nodes)
{
    ...
    for (i = 0; i < chat->num_tcp_nodes && i < max_nodes; ++i)
        nodes[i] = chat->tcp_nodes[i];
...
static void update_gc_addresses_cb(GC_Announce *announce, const uint8_t *chat_id, void *object)
{
    ...
    if (num_tcp_nodes) {
        add_gc_tcp_nodes(chat, tcp_nodes, num_tcp_nodes);
    }
}
...
/* Returns true if peernumber's public encryption key is closest to the Chat ID. */
static int is_closest_peer(const GC_Chat *chat, int peernumber)
{
    uint32_t i;
    ...
    for (i = 0; i < chat->numpeers; ++i) {
        ...
        if (id_closest(CHAT_ID(chat->chat_public_key), chat->gcc[peernumber].addr.public_key,
                               chat->gcc[i].addr.public_key) == 2) {
            return false;
        }
    }
...
static int send_gc_sync_request(GC_Chat *chat, uint32_t peernumber, uint32_t num_peers)
{
    ...
    if (!chat->num_tcp_nodes || chat->request_tcp_nodes)
                                  ...
                                  uint32_t length)
{
    ...
    if (send_peer_mod_list(chat, peernumber) == -1)
        return -1;

    if (send_peer_sanctions_list(chat, peernumber) == -1)
        return -1;
...
static unsigned int add_gc_tcp_nodes(GC_Chat *chat, const Node_format *nodes, unsigned int num_nodes)
{
    ...
    for (i = 0; i < num_nodes && i < MAX_GC_TCP_NODES; ++i) {
        add_tcp_relay_global(chat->tcp_conn, nodes[i].ip_port, nodes[i].public_key);
        memcpy(&chat->tcp_nodes[i], &nodes[i], sizeof(Node_format));
        fprintf(stderr, "added group TCP node: %s\n", id_toa(nodes[i].public_key));
    }

    chat->num_tcp_nodes = i;

    return i;
}

/* Attempts to add new TCP nodes to the group instance.
 *
...
{
    Node_format tcp_nodes[MAX_GC_TCP_NODES];
    unsigned int num = tcp_copy_connected_relays(m->net_crypto->tcp_c, tcp_nodes, MAX_GC_TCP_NODES);
    return add_gc_tcp_nodes(chat, tcp_nodes, num);
}
...
static int send_peer_group_tcp_relays(GC_Chat *chat, uint32_t peernumber)
{
    if (chat->num_tcp_nodes == 0)
        return -1;

    uint8_t data[HASH_ID_BYTES + sizeof(Node_format) * chat->num_tcp_nodes];
    U32_to_bytes(data, chat->self_public_key_hash);
    uint32_t length = HASH_ID_BYTES;

    int nodes_len = pack_nodes(data + length, sizeof(data) - length, chat->tcp_nodes, chat->num_tcp_nodes);
                                      ...
                                      uint32_t length)
{
    ...
    int i, j;
    /* Connect all peers to new nodes */
    for (i = 1; i < chat->numpeers; ++i) {
        for (j = 0; j < num_nodes; ++j) {
            add_tcp_relay_connection(chat->tcp_conn, chat->gcc[i].tcp_connection_num, tcp_relays[j].ip_port,
                                     tcp_relays[j].public_key);
        }
    }

    chat->request_tcp_nodes = false;
    chat->gcc[peernumber].pending_tcp_request = false;
    return 0;
}

/* Checks if we have valid group TCP nodes. If we do not, we will attempt to create a new
 * set of nodes for the group if we are the "chosen one". Otherwise we do nothing and
 * wait for the "chosen one" to perform this task and then sync with us.
 *
 * TODO: check actual connection status of nodes.
 */
#define GC_TCP_NODES_CHECK_INTERVAL 10
static void check_gc_tcp_nodes(const Messenger *m, GC_Chat *chat)
{
    if (chat->connection_state != CS_CONNECTED)
        return;

    if (!is_timeout(chat->last_tcp_nodes_check, GC_TCP_NODES_CHECK_INTERVAL))
        return;

    chat->last_tcp_nodes_check = unix_time();

    if (chat->num_tcp_nodes || get_gc_confirmed_numpeers(chat) <= 1)
        return;

    if (is_closest_peer(chat, 0)) {
        fprintf(stderr, "Getting new TCP node set\n");
        new_gc_tcp_nodes(m, chat);
    }
}
...
{
    uint32_t num_peers, sstate_version, screds_version, topic_version, num_tcp_nodes;
    bytes_to_U32(&num_peers, sync_data);
    ...
    bytes_to_U32(&num_tcp_nodes, sync_data + (sizeof(uint32_t) * 4));

    if (num_peers > get_gc_confirmed_numpeers(chat)
        ...
        || (num_tcp_nodes && !chat->num_tcp_nodes)) {
...
int gc_set_self_status(Messenger *m, int groupnumber, uint8_t status)
{
    ...
    if (c->status_change)
        (*c->status_change)(m, groupnumber, chat->gcc[0].peer_id, status, c->status_change_userdata);
                                   ...
                                   uint32_t length)
{
    ...
    if (c->status_change)
        (*c->status_change)(m, groupnumber, chat->gcc[peernumber].peer_id, status, c->status_change_userdata);
                                        ...
                                        const uint8_t *data, uint32_t length)
{
    ...
    if (c->peer_join && !chat->gcc[peernumber].confirmed)
        (*c->peer_join)(m, groupnumber, chat->gcc[peernumber].peer_id, c->peer_join_userdata);
...
on_error:
    gc_peer_delete(m, groupnumber, peernumber, "BAD SSTATE", 10);
...
/* Sends the moderator list to peernumber.
 *
 * Returns a non-negative value on success.
 * Returns -1 on failure.
 */
...
/* Sends the sanctions list to peernumber.
 *
 * Returns non-negative value on success.
 * Returns -1 on failure.
 */
static int send_peer_sanctions_list(GC_Chat *chat, uint32_t peernumber)
{
    ...
    return send_lossless_group_packet(chat, peernumber, packet, packet_len, GP_SANCTIONS_LIST);
}
...
int gc_set_self_nick(Messenger *m, int groupnumber, const uint8_t *nick, uint16_t length)
{
    ...
    if (c->nick_change)
        (*c->nick_change)(m, groupnumber, chat->gcc[0].peer_id, nick, length, c->nick_change_userdata);
                          ...
                          uint32_t length)
{
    ...
    if (c->nick_change)
        (*c->nick_change)(m, groupnumber, chat->gcc[peernumber].peer_id, nick, length, c->nick_change_userdata);
...
/* Sends the group topic to peernumber.
 *
 * Returns 0 on success.
 * Returns -1 on failure.
 */
static int send_peer_topic(GC_Chat *chat, uint32_t peernumber)
{
    ...
    if (send_lossless_group_packet(chat, peernumber, packet, packet_len, GP_TOPIC) == -1)
        return -1;
...
int gc_set_topic(GC_Chat *chat, const uint8_t *topic, uint16_t length)
{
    ...
    if (chat->topic_info.version != UINT32_MAX)   /* TODO: improbable, but an overflow would break everything */
...
static int send_gc_set_mod(GC_Chat *chat, uint32_t peernumber, bool add_mod)
{
...
/* Adds or removes peernumber from moderator list if add_mod is true or false respectively.
...
int founder_gc_set_moderator(GC_Chat *chat, uint32_t peernumber, bool add_mod)
{
    ...
    if (add_mod) {
        ...
        if (mod_list_add_entry(chat, SIG_PK(chat->gcc[peernumber].addr.public_key)) == -1)
            return -1;
    } else {
        if (mod_list_remove_entry(chat, SIG_PK(chat->gcc[peernumber].addr.public_key)) == -1)
            return -1;

        if (update_gc_sanctions_list(chat,  SIG_PK(chat->gcc[peernumber].addr.public_key)) == -1)
            return -1;

        if (update_gc_topic(chat, SIG_PK(chat->gcc[peernumber].addr.public_key)) == -1)
            return -1;
    }
    ...
    if (send_gc_set_mod(chat, peernumber, add_mod) == -1)
        return -1;
 ...
 * Returns -1 on failure.
 */
static int send_gc_set_observer(GC_Chat *chat, uint32_t peernumber, const uint8_t *sanction_data,
...
static int mod_gc_set_observer(GC_Chat *chat, uint32_t peernumber, bool add_obs)
{
    ...
    if (send_gc_set_observer(chat, peernumber, sanction_data, length, add_obs) == -1)
        return -1;
...
int gc_set_peer_role(Messenger *m, int groupnumber, uint32_t peer_id, uint8_t role)
{
    ...
    if (c->moderation)
        (*c->moderation)(m, groupnumber, chat->gcc[0].peer_id, chat->gcc[peernumber].peer_id, mod_event,
                             ...
                             uint32_t length, uint8_t type)
{
    ...
    if (c->message)
        (*c->message)(m, groupnumber, chat->gcc[peernumber].peer_id, cb_type, data, length, c->message_userdata);
                                        ...
                                        const uint8_t *data, uint16_t length)
{
    ...
    switch (request_type) {
        case HS_INVITE_REQUEST:
            ret = send_gc_invite_request(chat, peernumber);
            break;
        case HS_PEER_INFO_EXCHANGE:
            ret = send_gc_peer_exchange(m->group_handler, chat, peernumber);
            break;
        default:
                                      ...
                                      bool direct_conn)
{
    ...
    if (lossless_ret == 1) {
        ...
        return gc_send_message_ack(chat, peernumber, 0, gconn->recv_message_id + 1);
    }
 ...
 * return -1 on failure.
 */
static int handle_gc_lossy_message(Messenger *m, GC_Chat *chat, const uint8_t *packet, uint16_t length, bool direct_conn)
{
...
int gc_peer_delete(Messenger *m, int groupnumber, uint32_t peernumber, const uint8_t *data, uint16_t length)
{
    ...
    if (c->peer_exit && chat->gcc[peernumber].confirmed)
        (*c->peer_exit)(m, groupnumber, chat->gcc[peernumber].peer_id, data, length, c->peer_exit_userdata);

    kill_tcp_connection_to(chat->tcp_conn, chat->gcc[peernumber].tcp_connection_num);
...
static int peer_update(Messenger *m, int groupnumber, GC_GroupPeer *peer, uint32_t peernumber)
{
    ...
    if (nick_num != -1 && nick_num != peernumber) {   /* duplicate nick */
        if (c->peer_exit)
            (*c->peer_exit)(m, groupnumber, chat->gcc[peernumber].peer_id, NULL, 0, c->peer_exit_userdata);
...
static int peer_add(Messenger *m, int groupnumber, IP_Port *ipp, const uint8_t *public_key)
{
    ...
    gconn->peer_id = get_new_peer_id(chat);
...
static void ping_group(GC_Chat *chat)
{
    ...
    U32_to_bytes(data + HASH_ID_BYTES + (sizeof(uint32_t) * 4), (uint32_t) chat->num_tcp_nodes);
...
static void do_group_tcp(const Messenger *m, GC_Chat *chat)
{
    if (!chat->tcp_conn)
        return;

    check_gc_tcp_nodes(m, chat);
...
int gc_group_load(GC_Session *c, struct SAVED_GROUP *save)
{
    ...
    for (i = 0; i < num_addrs && num < MAX_GC_PEER_ADDRS; ++i) {
    ...
    add_gc_tcp_nodes(chat, save->tcp_nodes, ntohs(save->num_tcp_nodes));
    chat->request_tcp_nodes = true;
...
int gc_group_add(GC_Session *c, uint8_t privacy_state, const uint8_t *group_name, uint16_t length)
{
    ...
    new_gc_tcp_nodes(c->messenger, chat);
...
int gc_invite_friend(GC_Session *c, GC_Chat *chat, int32_t friendnumber)
{
    ...
    if (chat->numpeers <= 1 && chat->num_tcp_nodes == 0)
        new_gc_tcp_nodes(c->messenger, chat);

    uint8_t data[1 + CHAT_ID_SIZE + sizeof(Node_format) + (sizeof(Node_format) * chat->num_tcp_nodes)];
    data[0] = GP_FRIEND_INVITE;
    memcpy(data + 1, CHAT_ID(chat->chat_public_key), CHAT_ID_SIZE);
    ...
    int tcp_nodes_size = pack_nodes(data + length, sizeof(data) - length, chat->tcp_nodes, chat->num_tcp_nodes);
    ...
    if (send_group_invite_packet(c->messenger, friendnumber, data, length) == -1)
        return -3;
...
int gc_accept_invite(GC_Session *c, const uint8_t *data, uint16_t length, const uint8_t *passwd, uint16_t passwd_len)
{
    ...
    if (unpack_nodes(&node, 1, &node_len, data + CHAT_ID_SIZE, length - CHAT_ID_SIZE, 0) != 1)
        return -1;

    Node_format tcp_nodes[MAX_GC_TCP_NODES];
    int num_tcp_nodes = unpack_nodes(tcp_nodes, MAX_GC_TCP_NODES, 0, data + CHAT_ID_SIZE + node_len,
                                     length - CHAT_ID_SIZE - node_len, 1);
    if (num_tcp_nodes == -1)
        return -1;
